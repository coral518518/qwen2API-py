...
function IconButton({ children, title, onClick, disabled, danger }: { children: ReactNode; title: string; onClick: () => void; disabled?: boolean; danger?: boolean }) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`inline-grid size-9 place-items-center rounded-xl border border-transparent text-muted-foreground transition hover:border-white/75 hover:bg-muted hover:text-foreground disabled:pointer-events-none disabled:opacity-45 ${danger ? "hover:text-rose-600" : ""
        }`}
    >
      {children}
    </button>
