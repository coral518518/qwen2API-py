...
export default function AdminLayout() {
  ...
  return (
    <div className="relative flex h-screen w-full overflow-hidden text-foreground">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-screen w-[16.5rem] flex-col border-r border-white/70 bg-card/92 shadow-[var(--shadow-lift)] backdrop-blur-2xl transition-transform duration-300 md:relative md:inset-auto md:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
          ...
          {navItems.map(item => {
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={() => setMobileOpen(false)}
                className={`group flex items-center gap-3 whitespace-nowrap rounded-2xl border px-4 py-3 text-sm font-bold transition ${active
              ...
              >
                <span
                  className={`grid size-9 shrink-0 place-items-center rounded-xl transition ${active ? "bg-primary-foreground/12" : "bg-muted/45 group-hover:bg-accent/55"
                    }`}
                >
