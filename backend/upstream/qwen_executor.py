...
class QwenExecutor:
    ...
    async def create_chat(self, token: str, model: str, chat_type: str = "t2t") -> str:
        ...
        if self.chat_id_pool is not None and self.account_pool is not None:
            try:
                acc = next(
                    (a for a in self.account_pool.accounts if a.token == token), None
                )
                if acc is not None:
        ...
        request_fn = getattr(self.engine, "_request_json", None) or getattr(
            self.engine, "api_call", None
        )
        ...
        if r["status"] != 200:
            ...
            if (
                ...
                raise Exception(
                    f"unauthorized: create_chat HTTP {r['status']}: {body_text[:100]}"
                )
            if r["status"] == 429:
    ...
    async def stream(
        ...
        stream_fn = getattr(self.engine, "stream_chat_once", None) or getattr(
            self.engine, "fetch_chat", None
        )
        ...
        log.info(
            f"[上游] 开始流式 会话={chat_id} 模型={model} 自定义工具={has_custom_tools} prompt长度={prompt_len} ({prompt_len / 1024:.1f}KB)"
        )
        log.info(
            f"[上游] 功能配置: function_calling={feature_config.get('function_calling')} auto_search={feature_config.get('auto_search')} code_interpreter={feature_config.get('code_interpreter')} plugins_enabled={feature_config.get('plugins_enabled')}"
        )
        ...
        if has_custom_tools and total_output_chars < 20 and elapsed > 5.0:
            log.warning(
                f"[上游] 异常短回复 仅 {total_output_chars} 字符 耗时 {elapsed:.1f}s — 疑似上游超时"
            )
            raise Exception(
                f"Upstream timeout suspected: only {total_output_chars} chars in {elapsed:.1f}s"
            )

        log.info(
            f"[上游] 流结束 会话={chat_id} 总耗时={elapsed:.3f}s 流字节={total_output_chars}"
        )
    ...
    async def chat_stream_events_with_retry(
        ...
        for attempt in range(settings.MAX_RETRIES):
            ...
            if not acc:
                raise Exception(
                    "No available accounts in pool (all busy or rate limited)"
                )

            try:
                log.info(
                    f"[上游] 账号已获取 账号={acc.email} 模型={model} 第{attempt + 1}次 获取耗时={acquire_elapsed:.3f}s"
                )
                ...
                update_request_context(chat_id=chat_id)
                log.info(
                    f"[上游] 创建会话 会话={chat_id} 账号={acc.email} 耗时={create_elapsed:.3f}s"
                )
                yield {"type": "meta", "chat_id": chat_id, "acc": acc}
                ...
                is_timeout = (
                    ...
                    or type(e).__name__
                    in ("ReadTimeout", "TimeoutError", "TimeoutException")
                )

                if is_timeout:
                    log.warning(
                        f"[上游] 超时 第{attempt + 1}/{settings.MAX_RETRIES}次 账号={acc.email} 错误={e}"
                    )
        ...
        raise Exception(
            f"All {settings.MAX_RETRIES} attempts failed. Please check upstream accounts."
        )
