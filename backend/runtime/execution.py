...
def should_force_finish_after_tool_use(
    stop_reason: str, trailing_idle_seconds: float, visible_output_after_tool: bool
) -> bool:
    return (
        stop_reason == "tool_use"
        and trailing_idle_seconds >= TRAILING_IDLE_AFTER_TOOL_SECONDS
        and not visible_output_after_tool
    )


def extract_blocked_tool_names(
    text: str, allowed_tool_names: list[str] | None = None
) -> list[str]:
    if not text:
        return []
...
def has_recent_unchanged_read_result(messages: list[dict[str, Any]] | None) -> bool:
    return any(
        "Unchanged since last read" in text for text in _recent_message_texts(messages)
    )
...
def has_recent_search_no_results(messages: list[dict[str, Any]] | None) -> bool:
    for text in _recent_message_texts(messages):
        ...
        if "websearch" not in lowered:
            continue
        if (
            "did 0 searches" in lowered
            or '"results": []' in lowered
            or '"matches": []' in lowered
        ):
            return True
    return False
...
def recent_same_tool_identity_count(
    messages: list[dict[str, Any]] | None, tool_name: str, tool_input: Any = None
) -> int:
    ...
    for msg in reversed(messages or []):
        ...
        if (
            len(tools) == 1
            and tool_identity(tools[0].get("name", ""), tools[0].get("input", {}))
            == target
        ):
            count += 1
            continue
...
def has_recent_openai_same_tool_call(
...
def has_invalid_textual_tool_contract(answer_text: str) -> bool:
    ...
    tc_m = re.search(
        r"##TOOL_CALL##\s*(.*?)\s*##END_CALL##", compact, re.DOTALL | re.IGNORECASE
    )
    if tc_m:
        try:
            ...
            return True
        tool_input = obj.get(
            "input", obj.get("args", obj.get("arguments", obj.get("parameters", {})))
        )
        return isinstance(tool_input, str)
    xml_m = re.search(
        r"<tool_call>\s*(.*?)\s*</tool_call>", compact, re.DOTALL | re.IGNORECASE
    )
    if xml_m:
        try:
    ...
    *,
    ...
    def _finalize_result(*, reason: str | None = None) -> RuntimeExecutionResult:
        ...
        if not detected_tool_calls and request.tools and answer_text:
            # 尝试从最终文本中解析工具调用
            tool_blocks, stop_reason = tool_parser.parse_tool_calls_silent(
                answer_text, request.tools
            )
        ...
        # 检查空输出
        if (
            not detected_tool_calls
            and not answer_text.strip()
            ...
            try:
                pool = getattr(client, "executor", None) and getattr(
                    client.executor, "chat_id_pool", None
                )
        ...
        state = RuntimeAttemptState(
            ...
            blocked_tool_names=extract_blocked_tool_names(
                answer_text.strip(), request.tool_names
            ),
        ...
        if phase == "answer" and content:
            ...
            if (
                ...
                if _TOXIC_REFUSAL_RE.search(early_answer):
                    toxic_blocked = extract_blocked_tool_names(
                        early_answer, request.tool_names
                    )
                ...
                if (
                    ...
                    if blocked_tool_names:
                        return _finalize_result(
                            reason=f"blocked_tool_name:{blocked_tool_names[0]}"
                        )
                if "##TOOL_CALL##" in answer_text or "<tool_call>" in answer_text:
...
def parse_tool_directive_once(
    request: StandardRequest, state: RuntimeAttemptState
) -> RuntimeToolDirective:
    if state.tool_calls:
    ...
    if request.tools and state.answer_text:
        tool_blocks, stop_reason = tool_parser.parse_tool_calls_silent(
            state.answer_text, request.tools
        )
        return RuntimeToolDirective(tool_blocks=tool_blocks, stop_reason=stop_reason)

    return RuntimeToolDirective(
        tool_blocks=[{"type": "text", "text": state.answer_text}],
...
async def collect_completion_run_with_recovery(
    ...
    *,
    ...
    if warmup_chars > 0 and on_delta is not None:
        ...
        async def _wrapped(evt, text_chunk, tool_calls):
            ...
            if (
                ...
                or evt.get("phase") not in ("answer", "text")
            ):
    ...
    while continues < max_continuation:
        ...
        assistant_ctx, followup = build_continuation_prompt(
            state.answer_text, anchor_chars=2000
        )
        ...
        merged_state = RuntimeAttemptState(
            ...
            blocked_tool_names=cont_result.state.blocked_tool_names
            or state.blocked_tool_names,
            finish_reason=cont_result.state.finish_reason or state.finish_reason,
            raw_events=state.raw_events,
            emitted_visible_output=state.emitted_visible_output
            or cont_result.state.emitted_visible_output,
            stage_metrics=state.stage_metrics,
        )
        result = RuntimeExecutionResult(
            state=merged_state, chat_id=result.chat_id, acc=result.acc
        )
        log.info(
...
def anthropic_stream_stop_reason(
    request: StandardRequest, state: RuntimeAttemptState, pending_chunks: list[str]
) -> str:
    if state.tool_calls or any(
        '"type": "tool_use"' in chunk for chunk in pending_chunks
    ):
        return "tool_use"
    return build_tool_directive(request, state).stop_reason


def finalize_anthropic_stream_success(
    *,
    request: StandardRequest,
    prompt: str,
    execution: RuntimeExecutionResult,
    ...
    stop_reason = anthropic_stream_stop_reason(
        request, execution.state, translator.pending_chunks
    )
    chunks = translator.finalize(
        answer_text=execution.state.answer_text, stop_reason=stop_reason
    )
...
async def complete_anthropic_stream_success(
    *,
    users_db,
    token: str,
    client,
...
def retryable_usage_delta(prompt: str):
    return lambda execution, current_prompt=None: (
        len(execution.state.answer_text) + len(current_prompt or prompt)
    )


def build_usage_delta_factory(
    prompt: str,
) -> Callable[[RuntimeExecutionResult, Any | None], int]:
    return lambda execution, current_prompt=None: (
        len(execution.state.answer_text) + len(current_prompt or prompt)
    )
    ...
    *,
    ...
    if attempt_index >= max_attempts - 1:
        return RuntimeRetryDirective(
            retry=False, next_prompt=current_prompt, reason=None
        )

    can_retry_after_output = (
        allow_after_visible_output or not state.emitted_visible_output
    )
    ...
    if state.blocked_tool_names and request.tools:
        ...
        blocked_name = normalize_tool_name(
            state.blocked_tool_names[0], request.tool_names
        )
        return _retry(
            ...
            tool_parser.inject_format_reminder(
                ...
                client_profile=getattr(
                    request, "client_profile", CLAUDE_CODE_OPENAI_PROFILE
                ),
            ),
        )
    ...
    if request.tools:
        ...
        if state.answer_text:
            ...
            if saw_contract_markup and can_retry_after_output:
                if has_invalid_textual_tool_contract(state.answer_text):
                    fallback_tool_name = (
                        request.tool_names[0] if request.tool_names else "tool"
                    )
                    return _retry(
        ...
        if directive.stop_reason == "tool_use":
            first_tool = next(
                (b for b in directive.tool_blocks if b.get("type") == "tool_use"), None
            )
            if first_tool:
                repeated_same_tool = False
                if (
                    getattr(request, "client_profile", CLAUDE_CODE_OPENAI_PROFILE)
                    == "openclaw_openai"
                ):
            ...
            if (
                ...
                if can_retry_after_output:
                    ...
                    log.warning(
                        f"[Runtime] Blocked repeated Read after 'Unchanged since last read', but cannot retry"
                    )
...
async def continue_after_retry_directive(
    *, client, execution, retry: RuntimeRetryDirective, preserve_chat: bool = False
) -> RuntimeRetryContinuation:
    if not retry.retry:
        return RuntimeRetryContinuation(
            should_continue=False, next_prompt=retry.next_prompt
        )
    await cleanup_runtime_resources(
        client, execution.acc, execution.chat_id, preserve_chat=preserve_chat
    )
...
async def cleanup_runtime_resources(
    client, acc, chat_id: str | None, *, preserve_chat: bool = False
) -> None:
    if acc is None:
        return
    ...
    if chat_id and token:
        ...
        async def _delete_chat_later() -> None:
            try:
                ...
                log.debug(
                    "[Cleanup] delete_chat failed chat_id=%s error=%s", chat_id, exc
                )
